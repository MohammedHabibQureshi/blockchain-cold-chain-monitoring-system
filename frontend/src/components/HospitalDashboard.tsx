...
export default function HospitalDashboard({
        ...
        try {
            ...
            const [usersRes, shipmentsRes, countRes] =
                await Promise.all([
                    getUsers(),
                    getShipments(),
                    getRoleCount("HOSPITAL")
                ]);

            setUsers(usersRes.data);
            setShipments(shipmentsRes.data);
            setHospitalCount(countRes.data.count);
    ...
    const closeShipment = async (shipmentId: string) => {
        ...
        try {
            ...
            const { txHash, blockNumber } =
                await runTransaction(
                    contract.closeShipment(shipmentId)
                );

            setTx({
                status: "success",
                message: `Shipment ${shipmentId} confirmed and completed.`,
                txHash,
                blockNumber
            });
    ...
    return (
        <div>
            <div className="dashboard-head">
                ...
                <WalletInfo
                    wallet={wallet}
                    roleLabel={ROLE_LABELS[6]}
                    roleEmoji={ROLE_EMOJIS[6]}
                />
            </div>
            ...
            <Panel title="🏥 Shipments In My Custody">
                <div className="panel-scroll">
                    {inCustody.length === 0 ? (
                        <Empty message="No shipments currently awaiting your inspection." />
                    ) : (
                        <div className="admin-items-grid">
            ...
            {completed.length > 0 && (
                <Panel title="✅ Completed Shipments">
                    <div className="panel-scroll">
                        <div className="admin-items-grid">
            ...
            {selected && (
                <div className="detail-wrapper">
                    ...
                    <ShipmentDetail
                        ...
                        closeLabel="✅ Confirm Receipt & Complete Shipment"
                        onClose={async () => {
                            await closeShipment(selected);
                            setSelected(null);
                        }}
                        showCancel={
                            inCustody.some(
